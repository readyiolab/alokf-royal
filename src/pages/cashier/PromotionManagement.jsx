// ============================================
// FILE: pages/cashier/PromotionManagement.jsx
// Promotion Management Page for Cashiers - Deposit Bonus System
// ============================================
...
const PromotionManagement = () => {
  ...
  // Check if user is admin or cashier (both can create/edit/delete promotions)
  const isAdmin = user?.role === 'admin';
  const isCashier = user?.role === 'cashier';
  const canManagePromotions = isAdmin || isCashier;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!canManagePromotions) {
      toast.error('Only admins and cashiers can create or edit promotions');
      return;
    }
  ...
  const handleDelete = async (promotionId) => {
    if (!canManagePromotions) {
      toast.error('Only admins and cashiers can delete promotions');
      return;
    }
  ...
  const handleEdit = (promotion) => {
    if (!canManagePromotions) {
      toast.error('Only admins and cashiers can edit promotions');
      return;
    }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">View and manage deposit bonus promotions</p>
          </div>
          <Button 
            onClick={() => { 
              if (!canManagePromotions) {
                toast.error('Only admins and cashiers can create promotions');
                return;
              }
              resetForm(); 
              setEditingPromotion(null); 
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-gray-500 mb-4">No promotions created yet</p>
              <Button 
                className="mt-4" 
                onClick={() => { 
                  if (!canManagePromotions) {
        ...
        {/* Create/Edit Dialog - For Admins and Cashiers */}
        {canManagePromotions && (
